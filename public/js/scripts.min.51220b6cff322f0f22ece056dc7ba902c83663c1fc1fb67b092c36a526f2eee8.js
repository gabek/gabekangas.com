...
return lunr.QueryParser.parsePresence
default:var errorMessage="Unexpected lexeme type '"+nextLexeme.type+"'"
throw new lunr.QueryParseError(errorMessage,nextLexeme.start,nextLexeme.end)}};(function(root,factory){if(typeof define==='function'&&define.amd){define(factory)}else if(typeof exports==='object'){module.exports=factory()}else{root.lunr=factory()}}(this,function(){return lunr}))})();;var idx=null;var resultDetails=[];var $searchResults;var $searchInput;window.onload=function(){var request=new XMLHttpRequest();var query='';$searchResults=document.getElementById('search-results');$searchInput=document.getElementById('search-input');query=getParameterByName('q')?getParameterByName('q').trim():'';request.overrideMimeType('application/json');request.open('GET','/index.json',true);request.onload=function(){if(request.status>=200&&request.status<400){var documents=JSON.parse(request.responseText);idx=lunr(function(){this.ref('ref');this.field('title');this.field('excerpt');this.field('body');documents.forEach(function(doc){this.add(doc);resultDetails[doc.ref]={title:doc.title,excerpt:doc.excerpt};},this);});if(query!=''){$searchInput.value=query;renderSearchResults(search(query));}}else{$searchResults.innerHTML='Error loading search results';}};request.onerror=function(){$searchResults.innerHTML='Error loading search results';};request.send();registerSearchHandlers();};function registerSearchHandlers(){$searchInput.oninput=function(event){var query=event.target.value;var results=search(query);renderSearchResults(results);if($searchInput.value==''){$searchResults.style.display='none';$searchResults.innerHTML='';}};}
function search(query){return idx.search(query);}
function renderSearchResults(results){$searchResults.style.display='block';var ul=document.createElement('ul');if(results.length>0){results.forEach(function(result){var li=document.createElement('li');li.classList.add('card');li.innerHTML='<div class="card-header"><a href="'+
result.ref+
'">'+
resultDetails[result.ref].title+
'</a></div>';ul.appendChild(li);});while($searchResults.hasChildNodes()){$searchResults.removeChild($searchResults.lastChild);}}else{$searchResults.innerHTML='No results found';}
$searchResults.appendChild(ul);}
function getParameterByName(name,url){if(!url)url=window.location.href;name=name.replace(/[\[\]]/g,'\\$&');var regex=new RegExp('[?&]'+name+'(=([^&#]*)|&|#|$)'),results=regex.exec(url);if(!results)return null;if(!results[2])return '';return decodeURIComponent(results[2].replace(/\+/g,' '));};Array.prototype.forEach.call(document.querySelectorAll('.mdl-card__media'),function(el){var link=el.querySelector('a');if(!link){return;}
var target=link.getAttribute('href');if(!target){return;}
el.addEventListener('click',function(){location.href=target;});});;document.addEventListener('DOMContentLoaded',function(){populateMastodon()},false)
function populateMastodon(){const url="https://mastodon.social/api/v1/accounts/271966/statuses?exclude_replies=true"
fetch(url).then(function(response){return response.json();}).then(function(myJson){const filtered=myJson.filter(function(item){console.log(item)
return!item.in_reply_to_id&&!item.sensitive&&item.mentions==0&&item.spoiler_text===""&&item.media_attachments.length==0&&!item.muted&&!item.locked&&!item.reblog})
if(filtered.length<5){document.getElementById("mastodon-container").style.display='none'
return}
const newDOMItems=filtered.map(function(item){return makeMastodonDOMItem(item)}).slice(0,4)
newDOMItems.forEach(element=>{document.getElementById("mastodon-content").appendChild(element)});document.getElementById("mastodon-content")
console.log(filtered)}).catch(error=>document.getElementById("mastodon-container").style.display='none')}
function makeMastodonDOMItem(item){let node=document.createElement("div")
node.className="sidebar-item"
node.innerHTML=`${item.content}<hr>`
return node};document.addEventListener('DOMContentLoaded',function(){populateTopArtists()},false)
function populateTopArtists(){const url="https://homepageproxy.gabekangas.com/artists/artists.go"
fetch(url).then(function(response){return response.json();}).then(function(artists){if(artists.length===0){document.getElementById("topartists-container").style.display='none'
return}
const newDOMItems=artists.map(function(item){return makeTopArtistDOMItem(item)})
newDOMItems.forEach(element=>{document.getElementById("topartists-content").appendChild(element)});}).catch(error=>document.getElementById("topartists-container").style.display='none')}
function makeTopArtistDOMItem(item){let date=new Date(item.created_at)
let node=document.createElement("span")
node.className=`tag_font_size_${item.count} topartist`
node.innerHTML=`<a href="${item.url}">${item.name}</a>&nbsp;`
return node};document.addEventListener('DOMContentLoaded',function(){populateConcerts()},false)
function populateConcerts(){const url="https://homepageproxy.gabekangas.com/concerts/concerts.go"
fetch(url).then(function(response){return response.json();}).then(function(response){if(response.concerts.length===0){document.getElementById("concerts-container").style.display='none'
return}
const newDOMItems=response.concerts.map(function(item){return makeConcertDOMItem(item)})
newDOMItems.forEach(element=>{document.getElementById("concerts-content").appendChild(element)});}).catch(error=>document.getElementById("concerts-container").style.display='none')}
function makeConcertDOMItem(item){let node=document.createElement("li")
node.innerHTML=`<a href="${item.url}">${item.name}</a>`
return node}